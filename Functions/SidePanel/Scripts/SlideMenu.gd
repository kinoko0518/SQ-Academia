...
func update_timetable_display():
	...
	for _i in range(get_timetable_from_weekday(get_slippaged_date()).size()):
		var class_indicator_temp:Control = preload("res://Functions/SidePanel/UIs/class_display.tscn").instantiate()
...
func _on_turn_on_debug_mode_button_down():
	if __debug__ < 5:
		__debug__ += 1
	else:
		...
		if err != OK:
			push_error("Invalid config")
		
		config.set_value("global", "is_debug", true)
		
		$"../Features/MarginContainer/ScrollContainer/MarginContainer/VBoxContainer/DebugFeaturesLabel".visible = config.get_value("global", "is_debug")
		$"../Features/MarginContainer/ScrollContainer/MarginContainer/VBoxContainer/DebugFeatures".visible = config.get_value("global", "is_debug")
